...
-- Note: schema_name is GENERATED ALWAYS AS (computed from tenant_id), so Postgres
-- already prevents manual updates. We only need to check tenant_id.
...
BEGIN
  ...
  END IF;
  -- schema_name check removed: it's a GENERATED column derived from tenant_id
  RETURN NEW;
END;
$$;
