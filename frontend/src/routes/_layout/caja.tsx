...
function Caja() {
    ...
    /* [MODIFIED] Load directly from Supabase */
    const loadData = async () => {
        // 1. Fetch Orders 'por_cobrar' (pagando)
        ...
        `)
            .eq('tenant_id', 1) // [FIX] Ensure we filter/select by tenant to satisfy RLS
            .in('status', ['pagando', 'delivered', 'ready']); // Adjust based on flow. 'servir' -> 'entregado' -> 'pagando'
        ...
        if (ordersData) {
            setPendingOrders(ordersData.map((o: any) => ({
                id: o.id,
                tableId: `t-${o.table_id}`,
                status: o.status === 'paying' ? 'pagando' : 'por_cobrar', // Map appropriately
                total: o.total,
                timestamp: new Date(o.created_at).getTime(),
                items: o.items.map((i: any) => ({
                    productId: i.id, // or product_id
                    productName: i.product?.name || 'Item',
                    price: i.price,
                    quantity: i.quantity,
                })),
                isSplit: false // Handle split logic if DB supports it
            })));
        }

        // 2. Fetch Products for "Extras" search
        const { data: prodsData } = await supabase.from('products').select('*').eq('is_active', true);
        if (prodsData) {
            setAllProducts(prodsData.map((p: any) => ({
                id: p.id,
                name: p.name,
                price: p.price,
    ...
    const handlePay = async () => {
        if (selectedOrder) {
            ...
            try {
                ...
                // 1. Procesar Factura Electrónica (Si se requiere) - PRIORIDAD
                if (needsInvoice) {
                    try {
                        ...
                        toast(`Error Factura: ${billingError.message || 'Error desconocido'}`, "warning");
                        // No bloqueamos el pago si falla la factura, pero avisamos.
                        // Opcional: return; para bloquear.
                    }
                }

                // 2. Registrar Pago en Supabase (Actualizar Orden)
                // Reemplazo de MockService.payOrder
                const { error: updateError } = await supabase
                    .from('orders')
                    .update({
                        status: 'pagado', // o 'pagada' segun enum DB
                        total: total,
                        payment_method: paymentMethod, // snake_case mapping
                        tip: tip,
                        // Si tienes columna para factura:
                        invoice_number: invoiceData?.invoice_number || null,
                        invoice_url: invoiceData?.qr_url || null,
                        invoice_cufe: invoiceData?.cufe || null
                    })
                    .eq('id', selectedOrder.id); // Asegurarse que ID coincida (number vs uuid)

                if (updateError) {
                    console.error("Supabase Error:", updateError);
                    throw new Error("Error registrando pago en base de datos");
                }
