    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                ...
                category_id: editingProd.category || 'fuertes',
                is_active: true
                // recipe: editingProd.recipe // Handle recipe save if logic exists
            };

            const { error } = await supabase
                .from('products')
                .upsert({
                    ...(editingProd.id ? { id: editingProd.id } : {}),
                    ...payload
                });
            ...
            toast("Error guardando producto: " + e.message, "error");
        }
    };
...
const HRTab = ({ toast }: { toast: any }) => {
    ...
    const toggleShift = async (user: User) => {
        ...
        await supabase.from('users').update({ is_active_shift: newStatus }).eq('id', user.id);
        loadUsers();
...
const TablesTab = ({ toast }: { toast: any }) => {
    ...
    useEffect(() => {
        // [MODIFIED] Use Supabase for tables
        const fetchTables = async () => {
            ...
            if (data) {
                setTables(data.map((t: any) => ({
                    id: t.id,
                    number: t.number,
                    status: t.status, // Ensure DB has 'libre','cocinando' etc matching types or map it
                    timestamp: t.last_updated ? new Date(t.last_updated).getTime() : undefined
                })));
            }
        };

        const i = setInterval(fetchTables, 2000);
        fetchTables();
