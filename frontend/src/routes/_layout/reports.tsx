...
import {
    ...
    ShoppingBag, CreditCard, PieChart, Users, Package,
...
function Reports() {
    ...
    const productPerformance = useMemo(() => {
        ...
        filteredSales.forEach(sale => {
            sale.itemsSummary?.forEach((item: { name: string; quantity: number; price: number }) => {
    ...
    const downloadExcel = () => {
        ...
        if (activeTab === 'sales') {
            const ws = XLSX.utils.json_to_sheet(filteredSales.map(s => ({
                ...
                Detalle_Items: s.itemsSummary?.map((i: { quantity: number; name: string }) => `${i.quantity}x ${i.name}`).join(', ')
            })));
                                        ...
                                        {filteredSales.length === 0 ? <tr><td colSpan={3} className="p-8 text-center text-muted-foreground italic">No hay datos.</td></tr> :
                                            filteredSales.slice().reverse().map((s) => (
                                                <tr key={s.id} className="hover:bg-muted/30">
                                                    ...
                                                    <td className="p-3"><div className="font-bold text-foreground">Mesa {s.tableNumber}</div><div className="text-xs text-muted-foreground truncate max-w-[200px]">{s.itemsSummary?.map((i: { quantity: number; name: string }) => `${i.quantity}x ${i.name}`).join(', ') || 'Varios'}</div></td>
