...
El sistema opera bajo una arquitectura híbrida que maximiza la agilidad del frontend y la robustez del backend para procesos críticos.
...
graph TD
    ...
    Supabase -- "Triggers & Webhooks" --> EdgeFunctions
...
Infraestructura Serverless que maneja la persistencia y reglas de negocio en tiempo real.
- **Base de Datos**: PostgreSQL con extensiones.
- **Seguridad**: Row Level Security (RLS) para aislamiento estricto de tenants.
- **Auth**: Supabase Auth (JWT).
- **Lógica**: Edge Functions (Deno) para operaciones de baja latencia.
...
```
/
├── frontend/               # Código fuente de la PWA
│   ├── src/
...
│   └── package.json
│
├── backend-facturacion/    # Microservicio de Facturación
│   ├── app/
│   │   ├── core/           # Configuración y seguridad (Crypto, JWT)
│   │   ├── db/             # Modelos SQLModel y conexión
│   │   ├── routers/        # Endpoints de la API
│   │   └── services/       # Lógica con Factus (Facturación, Notas Crédito)
...
- **Comandas (KDS)**: Sincronización en tiempo real entre sala y cocina.
- **Inventario**: Control de stock, recetas y escandallos.
...
# Base de Datos (SQLite por defecto para dev)
...
El sistema utiliza RLS nativo de PostgreSQL. Cada consulta a la base de datos incluye automáticamente el contexto del usuario autenticado, asegurando que un restaurante **nunca** pueda acceder a los datos de otro.
...
- El servicio valida cada petición entrante verificando el JWT de Supabase, asegurando que la solicitud provenga de un usuario autenticado y autorizado.

## 🛠️ Scripts de Utilidad

El backend incluye scripts en `backend-facturacion/scripts/` para facilitar el desarrollo y pruebas:

- **`seed_restaurant.py`**:
  - Inicializa la base de datos local.
  - Crea o actualiza un restaurante de prueba con credenciales Sandbox de Factus.
  - Uso: `python -m scripts.seed_restaurant`

- **`test_sync.py`**:
  - Realiza una prueba de conexión y sincronización de rangos de numeración con la API local.
  - Uso: `python -m scripts.test_sync` (Requiere el servidor corriendo).