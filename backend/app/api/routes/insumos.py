...
from app.models import Insumo, InsumoCreate, InsumoPublic, InsumosPublic, InsumoUpdate, Message, Receta
...
def read_insumos(
    ...
    statement = select(Insumo).offset(skip).limit(limit)
...
def delete_insumo(
    ...
    if uso_en_receta:
        raise HTTPException(
            status_code=400, 
            detail="No puedes borrar este insumo porque se usa en un Plato del menú. Borra el plato primero."
        )
